...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  inngest-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  inngest-cli auth show\n  inngest-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: inngest-cli auth set <token>");
      return;
    }
